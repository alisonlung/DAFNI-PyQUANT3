"""
highimpacts_rail.py
Choropleth and flow maps of rail mode.
"""
...
def plot_CkDiffRail_quantiles(impacts_df, neti):
    df_mean = []

    #for each unqiue zone number, calculate mean score by zone that appeared in i/ j
    for i in neti:
        mean_score = impacts_df.loc[(impacts_df[' net_i'] == i) | (impacts_df[' net_j'] == i), 'CkDiffRail'].mean()
        subset_df = pd.DataFrame({' net_i': [i], 'CkDiffRail': [mean_score]})
    ...
    ax = df_mean.plot(column='CkDiffRail', scheme='Quantiles', k=8, cmap='inferno', legend=True, figsize=(9.0, 15.5), legend_kwds={'title': 'CkDiffRail', "fmt": "{:.0f}"})
    cx.add_basemap(ax, crs=df_mean.crs, source=cx.providers.CartoDB.PositronNoLabels)

    #export
    plt.savefig("outputs/rail/CkDiffRail_quantile.png", transparent=True)

def plot_savedSecsRail_quantiles(impacts_df, neti):
    df_mean = []

    #for each unqiue zone number, calculate mean score by zone that appeared in i/ j
    for i in neti:
        mean_score = impacts_df.loc[(impacts_df[' net_i'] == i) | (impacts_df[' net_j'] == i), ' savedSecsRail'].mean()
        subset_df = pd.DataFrame({' net_i': [i], ' savedSecsRail': [mean_score]})
        df_mean.append(subset_df)
    ...
    #plot with quantiles scheme
    ax = df_mean.plot(column=' savedSecsRail', scheme='Quantiles', k=8, cmap='inferno', legend=True, figsize=(9.0, 15.5), legend_kwds={'title': 'savedSecsRail', "fmt": "{:.0f}"})
    cx.add_basemap(ax, crs=df_mean.crs, source=cx.providers.CartoDB.PositronNoLabels)

    #export
    plt.savefig("outputs/rail/savedSecsRail_quantile.png", transparent=True)    

#Choropleth maps - FisherJenks
def plot_CkDiffRail_fisherjenks(impacts_df, neti):
    df_mean = []
    ...
    #plot with quantiles scheme 
    ax = df_mean.plot(column='CkDiffRail', scheme='FisherJenks', k=8, cmap='inferno', legend=True, figsize=(9.0, 15.5), legend_kwds={'title': 'CkDiffRail', "fmt": "{:.0f}"})
    cx.add_basemap(ax, crs=df_mean.crs, source=cx.providers.CartoDB.PositronNoLabels)

    #export
    plt.savefig("outputs/rail/CkDiffRail_fisherjenks.png", transparent=True)

def plot_savedSecsRail_fisherjenks(impacts_df, neti):
    df_mean = []
    ...
    #plot with quantiles scheme
    ax = df_mean.plot(column=' savedSecsRail', scheme='FisherJenks', k=8, cmap='inferno', legend=True, figsize=(9.0, 15.5), legend_kwds={'title': 'savedSecsRail', "fmt": "{:.0f}"})
    cx.add_basemap(ax, crs=df_mean.crs, source=cx.providers.CartoDB.PositronNoLabels)

    #export
    plt.savefig("outputs/rail/savedSecsRail_fisherjenks.png", transparent=True)    
...
def plot_CkDiffRail_flowmap(merged_df):
    ...
    tmax = top_1_df['CkDiffRail'].max()
    tmin = top_1_df['CkDiffRail'].min()
    ...
    for index, row in top_1_df.iterrows():
        ...
        arrow_color = cmap(norm(row['CkDiffRail']))
    ...
    cbar.set_label('CkDiffRail')

    #export
    plt.savefig("outputs/rail/CkDiffRail_flowmap.png", transparent=True)  

def plot_savedSecsRail_flowmap(merged_df):
    #group by net_i and retrieve top destination
    neti_groups = merged_df.groupby(' net_i')
    top_1_list = []
    for neti, group_data in neti_groups:
        top_1 = group_data.nlargest(1, ' savedSecsRail') 
        top_1_list.append(top_1)
    top_1_df = pd.concat(top_1_list)
    top_1_df = top_1_df.sort_values(' savedSecsRail')
    ...
    #set up colourmap
    cmap = plt.get_cmap("Spectral").reversed() 
    tmax = top_1_df[' savedSecsRail'].max()
    tmin = top_1_df[' savedSecsRail'].min()
    norm = Normalize(tmin, tmax)
    ...
    for index, row in top_1_df.iterrows():
        #calculate arrow properties
        arrow_color = cmap(norm(row[' savedSecsRail']))
        arrow_start = (row['centroid_i'].x, row['centroid_i'].y)
        arrow_end = (row['centroid_j'].x, row['centroid_j'].y)
        arrow_dx = arrow_end[0] - arrow_start[0]
        arrow_dy = arrow_end[1] - arrow_start[1]
    ...
    sm = ScalarMappable(norm=norm, cmap=cmap)
    cbar = plt.colorbar(sm, ax=ax, shrink = 0.9)
    cbar.set_label('savedSecsRail')

    #export
    plt.savefig("outputs/rail/savedSecsRail_flowmap.png", transparent=True)  
...
"""
#Local flow maps
def plot_CkDiffRail_localflowmap(merged_df, regions):
    #group by net_i and retrieve top destination
    neti_groups = merged_df.groupby(' net_i')
    top_1_list = []
    for neti, group_data in neti_groups:
    ...
    for bbox, region_name in regions:
        ...
        #export
        plt.savefig(f"outputs/rail/CkDiffRail_flowmap_{region_name}.png", transparent=True)  

def plot_savedSecsRail_localflowmap(merged_df, regions):
    #group by net_i and retrieve top destination
    neti_groups = merged_df.groupby(' net_i')
    top_1_list = []
    for neti, group_data in neti_groups:
...
dfs = []
for root, dirs, files in os.walk('inputs/model-runs/dafni_impacts_rail'):
    for file in files:
...
#plot
#choropleth maps
plot_CkDiffRail_quantiles(impacts_df, neti)
plot_savedSecsRail_quantiles(impacts_df, neti)
plot_CkDiffRail_fisherjenks(impacts_df, neti)
plot_savedSecsRail_fisherjenks(impacts_df, neti)
#flow maps
plot_CkDiffRail_flowmap(merged_df)
plot_savedSecsRail_flowmap(merged_df)
#local flow maps
plot_CkDiffRail_localflowmap(merged_df, regions)
plot_savedSecsRail_localflowmap(merged_df, regions)
