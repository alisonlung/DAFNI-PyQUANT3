"""
highimpacts_bus.py
Choropleth and flow maps of bus mode.
"""
...
def plot_CkDiffBus_quantiles(impacts_df, neti):
    df_mean = []

    #for each unqiue zone number, calculate mean score by zone that appeared in i/ j
    for i in neti:
        mean_score = impacts_df.loc[(impacts_df[' net_i'] == i) | (impacts_df[' net_j'] == i), 'CkDiffBus'].mean()
        subset_df = pd.DataFrame({' net_i': [i], 'CkDiffBus': [mean_score]})
    ...
    ax = df_mean.plot(column='CkDiffBus', scheme='Quantiles', k=8, cmap='inferno', legend=True, figsize=(9.0, 15.5), legend_kwds={'title': 'CkDiffBus', "fmt": "{:.0f}"})
    cx.add_basemap(ax, crs=df_mean.crs, source=cx.providers.CartoDB.PositronNoLabels)

    #export
    plt.savefig("outputs/bus/CkDiffBus_quantile.png", transparent=True)

def plot_savedSecsBus_quantiles(impacts_df, neti):
    df_mean = []

    #for each unqiue zone number, calculate mean score by zone that appeared in i/ j
    for i in neti:
        mean_score = impacts_df.loc[(impacts_df[' net_i'] == i) | (impacts_df[' net_j'] == i), ' savedSecsBus'].mean()
        subset_df = pd.DataFrame({' net_i': [i], ' savedSecsBus': [mean_score]})
        df_mean.append(subset_df)
    ...
    #plot with quantiles scheme
    ax = df_mean.plot(column=' savedSecsBus', scheme='Quantiles', k=8, cmap='inferno', legend=True, figsize=(9.0, 15.5), legend_kwds={'title': 'savedSecsBus', "fmt": "{:.0f}"})
    cx.add_basemap(ax, crs=df_mean.crs, source=cx.providers.CartoDB.PositronNoLabels)

    #export
    plt.savefig("outputs/bus/savedSecsBus_quantile.png", transparent=True)    

#Choropleth maps - FisherJenks
def plot_CkDiffBus_fisherjenks(impacts_df, neti):
    df_mean = []
    ...
    #plot with quantiles scheme 
    ax = df_mean.plot(column='CkDiffBus', scheme='FisherJenks', k=8, cmap='inferno', legend=True, figsize=(9.0, 15.5), legend_kwds={'title': 'CkDiffBus', "fmt": "{:.0f}"})
    cx.add_basemap(ax, crs=df_mean.crs, source=cx.providers.CartoDB.PositronNoLabels)

    #export
    plt.savefig("outputs/bus/CkDiffBus_fisherjenks.png", transparent=True)

def plot_savedSecsBus_fisherjenks(impacts_df, neti):
    df_mean = []
    ...
    #plot with quantiles scheme
    ax = df_mean.plot(column=' savedSecsBus', scheme='FisherJenks', k=8, cmap='inferno', legend=True, figsize=(9.0, 15.5), legend_kwds={'title': 'savedSecsBus', "fmt": "{:.0f}"})
    cx.add_basemap(ax, crs=df_mean.crs, source=cx.providers.CartoDB.PositronNoLabels)

    #export
    plt.savefig("outputs/bus/savedSecsBus_fisherjenks.png", transparent=True)    
...
def plot_CkDiffBus_flowmap(merged_df):
    ...
    tmax = top_1_df['CkDiffBus'].max()
    tmin = top_1_df['CkDiffBus'].min()
    ...
    for index, row in top_1_df.iterrows():
        ...
        arrow_color = cmap(norm(row['CkDiffBus']))
    ...
    cbar.set_label('CkDiffBus')

    #export
    plt.savefig("outputs/bus/CkDiffBus_flowmap.png", transparent=True)  

def plot_savedSecsBus_flowmap(merged_df):
    #group by net_i and retrieve top destination
    neti_groups = merged_df.groupby(' net_i')
    top_1_list = []
    for neti, group_data in neti_groups:
        top_1 = group_data.nlargest(1, ' savedSecsBus') 
        top_1_list.append(top_1)
    top_1_df = pd.concat(top_1_list)
    top_1_df = top_1_df.sort_values(' savedSecsBus')
    ...
    #set up colourmap
    cmap = plt.get_cmap("Spectral").reversed() 
    tmax = top_1_df[' savedSecsBus'].max()
    tmin = top_1_df[' savedSecsBus'].min()
    norm = Normalize(tmin, tmax)
    ...
    for index, row in top_1_df.iterrows():
        #calculate arrow properties
        arrow_color = cmap(norm(row[' savedSecsBus']))
        arrow_start = (row['centroid_i'].x, row['centroid_i'].y)
        arrow_end = (row['centroid_j'].x, row['centroid_j'].y)
        arrow_dx = arrow_end[0] - arrow_start[0]
        arrow_dy = arrow_end[1] - arrow_start[1]
    ...
    sm = ScalarMappable(norm=norm, cmap=cmap)
    cbar = plt.colorbar(sm, ax=ax, shrink = 0.9)
    cbar.set_label('savedSecsBus')

    #export
    plt.savefig("outputs/bus/savedSecsBus_flowmap.png", transparent=True)  
...
"""
#Local flow maps
def plot_CkDiffBus_localflowmap(merged_df, regions):
    #group by net_i and retrieve top destination
    neti_groups = merged_df.groupby(' net_i')
    top_1_list = []
    for neti, group_data in neti_groups:
    ...
    for bbox, region_name in regions:
        ...
        #set up colourmap
        cmap = plt.get_cmap("Spectral").reversed() 
        tmax = region_df['CkDiffBus'].max()
        tmin = region_df['CkDiffBus'].min()
        norm = Normalize(tmin, tmax)
        ...
        #export
        plt.savefig(f"outputs/bus/CkDiffBus_flowmap_{region_name}.png", transparent=True)  

def plot_savedSecsBus_localflowmap(merged_df, regions):
    #group by net_i and retrieve top destination
    neti_groups = merged_df.groupby(' net_i')
    top_1_list = []
    for neti, group_data in neti_groups:
    ...
    for bbox, region_name in regions:
        ...
        #set up colourmap
        cmap = plt.get_cmap("Spectral").reversed() 
        tmax = region_df[' savedSecsBus'].max()
        tmin = region_df[' savedSecsBus'].min()
        norm = Normalize(tmin, tmax)
        ...
        #export
        plt.savefig(f"outputs/bus/savedSecsBus_flowmap_{region_name}.png", transparent=True)  
...
dfs = []
for root, dirs, files in os.walk('inputs/model-runs/dafni_impacts_bus'):
    for file in files:
...
#plot
#choropleth maps
plot_CkDiffBus_quantiles(impacts_df, neti)
plot_savedSecsBus_quantiles(impacts_df, neti)
plot_CkDiffBus_fisherjenks(impacts_df, neti)
plot_savedSecsBus_fisherjenks(impacts_df, neti)
#flow maps
plot_CkDiffBus_flowmap(merged_df)
plot_savedSecsBus_flowmap(merged_df)
#local flow maps
plot_CkDiffBus_localflowmap(merged_df, regions)
plot_savedSecsBus_localflowmap(merged_df, regions)
